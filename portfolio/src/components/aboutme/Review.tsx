...
export const Review = ({ onClose }: ReviewProps) => {
  ...
  return (
    <div className="bg-[rgba(0,0,0,.65)] fixed inset-0 overflow-y-scroll px-10">
      <div className="max-w-screen-xl mx-auto py-32 ">
        ...
        >
          {review.map((el, index) => (
            <li key={index} className="bg-white rounded-[15px] p-4 flex-1">
              <div className="flex items-center gap-4">
                <img src={el.profile} alt="프로필" className="w-14" />
                <p className="flex flex-col font-semibold">
                  {el.name}{" "}
                  <span className="font-light text-label_light">{el.team}</span>
                </p>
              </div>
              <div className="text-label_normal py-4">{el.content}</div>
            </li>
          ))}
        </ul>
      </div>
